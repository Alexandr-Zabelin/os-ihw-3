...
void HandleTCPClient(int clntSocket)
{
    ...
    while (1)
    {
        ...
        if (recvBuffer[0] == 'E')
        {
            ...
            if (ansBuffer[0] == 'Y')
            {
                printf("Visitor %s enters %s gallery\n", recvBuffer + sepIdx, galleryName);
                ++numberOfVisitorsInGallery;
            }
            else
            {
                printf("Visitor %s is trying to enter %s gallery. But it is full\n", recvBuffer + sepIdx, galleryName);
            }
        }

        if (recvBuffer[0] == 'L')
        {
            printf("Visitor %s leaves %s gallery\n", recvBuffer + sepIdx, galleryName);
            ansBuffer[0] = 'Y';
            --numberOfVisitorsInGallery;
        }
